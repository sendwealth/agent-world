...
async fn test_fed_treaty_lifecycle() {
    ...
    // Get treaty
    let (status, body) =
        get_json(&app, &format!("/api/v1/federation/treaties/{}", treaty_id)).await;
    assert_eq!(status, StatusCode::OK);
...
async fn test_cross_world_interaction_federation_and_migration() {
    ...
    assert!(
        body["total_worlds"].as_u64().unwrap_or(0) >= 2,
        "summary should show >= 2 worlds, got: {:?}",
        body
    );
    ...
    assert!(
        ...
        "should have >= 1 completed migration"
    );
