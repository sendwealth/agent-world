...
> **版本**: v1.0（Phase 1） | **日期**: 2026-05-15 | **作者**: Nano (PM) | **状态**: 历史参考文档

> ⚠️ **文档状态说明**
>
> 本文档是 **Phase 1 阶段的产品设计规格**，记录了项目立项时的产品愿景、用户画像与功能设计。
> 项目现已发展到 **v1.1.0（Phase 4 完成）**，本文档未随版本同步更新，**仅作历史参考**。
>
> | 需求 | 查阅文档 |
> |------|---------|
> | 当前架构与实现状态 | [ARCHITECTURE.md](ARCHITECTURE.md)（v1.1.0，current） |
> | 版本路线图与计划 | [ROADMAP.md](ROADMAP.md) |
>
> **本文档的保留价值**：追溯产品决策的初衷与需求来源。文中章节编号（如 §4.4、§12.1）
> 仍被代码注释引用（`survival/instinct.py`、`organization/org.rs`、`auth/roles.rs` 等），
> 因此章节结构保持不变。
