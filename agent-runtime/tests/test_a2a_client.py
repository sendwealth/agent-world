...
class TestA2AClientLifecycle:
    ...
    @pytest.mark.asyncio
    async def test_connect_raises_on_unready_channel(self):
        """connect() should raise ConnectionError when channel_ready() times out."""
        client = A2AClient(make_config(timeout=0.1))

        mock_channel = MagicMock()
        mock_channel.close = AsyncMock()
        # Create a coroutine that never resolves (simulates server unavailable)
        async def _hang_forever():
            await asyncio.Future()  # never resolves

        mock_channel.channel_ready = _hang_forever
        with (
            patch(
                "agent_runtime.a2a.client.grpc.aio.insecure_channel",
                return_value=mock_channel,
...
class TestA2AClientSyncRPCs:
    ...
    async def test_send_message(self):
        ...
        with (
            patch("agent_runtime.a2a.client.grpc.aio") as mock_aio,
        ):
            mock_aio.insecure_channel.return_value.channel_ready = AsyncMock(return_value=None)
            await client.connect()
...
class TestA2AClientRetry:
    ...
    async def test_retries_on_unavailable(self):
        ...
        with (
            patch("agent_runtime.a2a.client.grpc.aio.insecure_channel") as mock_insecure,
        ):
            mock_insecure.return_value.channel_ready = AsyncMock(return_value=None)
            await client.connect()
            client._stub = MagicMock()
