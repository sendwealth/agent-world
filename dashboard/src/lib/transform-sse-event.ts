...
export function transformSSEEvent(raw: RawSSEEvent): WorldEvent {
  ...
  // Generate a unique id (UUID + timestamp — no module-level mutable state)
  const id = `evt-${Date.now()}-${crypto.randomUUID()}`;
...
function buildDescription(
  ...
  switch (type) {
    case "tick_advanced":
      return `Tick #${payload.tick ?? 0} 已推进`;
    case "agent_spawned":
      return `${name} 诞生了`;
    case "agent_dying":
    ...
    case "transaction_completed":
      return `${payload.from ?? "?"} → ${payload.to ?? "?"}: $${payload.amount ?? 0}`;
    case "balance_changed":
      return `${name} 余额变更: ${payload.old_balance ?? 0} → ${payload.new_balance ?? 0} (Tick #${payload.tick ?? 0})`;
    case "phase_changed":
      return `${name} 阶段变更: ${payload.old_phase ?? "?"} → ${payload.new_phase ?? "?"}`;
    case "rule_violated":
      return `${name} 违反规则: ${payload.rule ?? "未知规则"}`;
    case "snapshot_taken":
      return `快照已保存至 Tick #${payload.tick ?? 0}`;
    case "task_created":
      return `新任务 ${payload.task_id ?? "?"} (奖励: $${payload.reward ?? 0})`;
    case "task_claimed":
    ...
    case "reputation_changed":
      return `${name} 信誉变更: ${payload.old_reputation ?? 0} → ${payload.new_reputation ?? 0}`;
    case "skill_level_up":
      return `${name} 技能 ${payload.skill ?? "?"} 升至 ${payload.new_level ?? "?"} 级`;
    case "tax_collected":
      return `${payload.payer_id ?? "?"} 缴纳 ${payload.tax_kind ?? "?"} 税 $${payload.tax_amount ?? 0}`;
    case "treasury_distributed":
      return `国库分配 $${payload.total_amount ?? 0}`;
    ...
    case "coordination_task_created":
      return `团队任务 ${payload.task_id ?? "?"} 创建 (最多 ${payload.max_agents ?? 0} 人)`;
    ...
    case "coordination_task_completed":
      return `团队任务 ${payload.task_id ?? "?"} 完成 (${payload.contributor_count ?? 0} 人参与)`;
    ...
    case "investment_purchased":
      return `${payload.investor_id ?? "?"} 投资 $${payload.total_amount ?? 0}`;
    default:
