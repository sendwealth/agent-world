...
fn default_ticks_per_level() -> u64 { 20 }
fn default_transfer_ratio() -> f64 { 0.7 }
fn default_max_apprentices() -> u32 { 3 }
...
impl MentorshipSystem {
    ...
    pub fn establish(
        ...
        let key = (mentor_id.to_string(), apprentice_id.to_string(), skill_name.to_string());
        ...
        let active_count = self.active_index.keys()
        ...
        let target_level = ((mentor_skill_level as f64) * self.config.transfer_ratio).floor() as u32;
    ...
    pub fn progress_tick(
        ...
        let active_sessions: Vec<Uuid> = self.sessions.values()
        ...
        for session_id in active_sessions {
            let should_cancel = {
                ...
                let mentor_alive = agent_map.get(&session.mentor_id)
                    .map(|&i| agents[i].2.phase != AgentPhase::Dead)
                    .unwrap_or(false);
                let apprentice_alive = agent_map.get(&session.apprentice_id)
                    .map(|&i| agents[i].2.phase != AgentPhase::Dead)
                    .unwrap_or(false);

                // Check if mentor can still teach
                let mentor_can_teach = agent_map.get(&session.mentor_id)
                    .map(|&i| {
                ...
                let apprentice_can_learn = agent_map.get(&session.apprentice_id)
                    .map(|&i| {
                        let phase = agents[i].2.phase;
...
mod tests {
        ...
        (
            ...
            AgentRecord {
                ...
                skills: {
                    ...
                    if !skill.is_empty() {
                        m.insert(skill.to_string(), SkillRecord {
    ...
    fn test_duplicate_mentorship_rejected() {
        let mut sys = MentorshipSystem::default();
        sys.establish("mentor", "apprentice", "mining", 5, 0).unwrap();
