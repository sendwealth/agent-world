...
use crate::economy::investment::{
    ...
    FreezeProductRequest, InvestmentError, ListTransactionsQuery, SellSharesRequest,
...
pub async fn inv_create_product(
    ...
    let Some(ref inv) = state.investment_system else {
        return (
            ...
            Json(serde_json::json!({"error": "investment system not available"})),
        );
    };
    ...
    match sys.create_product(req) {
        ...
        Err(e) => (
            inv_error_to_status(&e),
            Json(serde_json::json!({"error": e.to_string()})),
        ),
    }
}

pub async fn inv_list_products(State(state): State<AppState>) -> impl IntoResponse {
    let Some(ref inv) = state.investment_system else {
...
pub async fn inv_buy_shares(
    ...
    match sys.buy_shares(req) {
        Ok((pos, tx)) => (
            StatusCode::OK,
            Json(serde_json::json!({"position": pos, "transaction": tx})),
        ),
        Err(e) => (
            inv_error_to_status(&e),
            Json(serde_json::json!({"error": e.to_string()})),
...
pub async fn inv_get_portfolio(
    ...
    (
        StatusCode::OK,
        Json(serde_json::json!(sys.get_portfolio(&investor_id))),
    )
}

pub async fn inv_leaderboard(State(state): State<AppState>) -> impl IntoResponse {
    let Some(ref inv) = state.investment_system else {
        return (StatusCode::SERVICE_UNAVAILABLE, Json(serde_json::json!([])));
    };
    let sys = inv.lock().await;
    (
        StatusCode::OK,
        Json(serde_json::json!(sys.get_leaderboard())),
    )
}
...
pub async fn inv_list_transactions(
    ...
    (
        StatusCode::OK,
        Json(serde_json::json!(sys.list_transactions(&query))),
    )
}
...
pub async fn inv_list_dividends(
    ...
    (
        StatusCode::OK,
        Json(serde_json::json!(sys.list_dividends(product_id.as_deref()))),
    )
}
